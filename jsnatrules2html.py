...
def ft_nat_fwr(arr_prefix, natfwrdict):
...
## convert show configuration security policy | display json
    ...
    for rscnt in range(rset_cnt) :
        ...
        ruleset = arr_prefix['security']['nat']['source']['rule-set'][rscnt]
...
def ft_srcnat_pool(arr_prefix, snatpooldict):

## convert show configuration security nat source | display json

    srcnatpool = []

    for snp in range(poolcnt) :

        snatpool = arr_prefix['security']['nat']['source']['pool'][snp]
...
if __name__ == '__main__':
    ...
    with open(outputfile,'w') as htmlfile:
        ...
        htmlfile.write('Commited on {} \n'.format(snatruledict['configuration']['@']['junos:commit-localtime']))

        snatpools = ft_srcnat_pool(arr_prefix, snatruledict)
        snatpool_in_html = json2html.convert( json = snatpools)
        htmlfile.write(snatpool_in_html)

        natruleList = ft_nat_fwr(arr_prefix, snatruledict)
        natruleList_in_html = json2html.convert( json = natruleList )
        htmlfile.write(natruleList_in_html)
